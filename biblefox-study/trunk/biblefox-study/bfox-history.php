<?php
	...
	class History
	{
		...
		function get_refs_for_id($id)
		{
			global $wpdb;
			$refs = new BibleRefs;
			$refs->push_sets($wpdb->get_results($wpdb->prepare("SELECT verse_start, verse_end FROM " . BFOX_TABLE_READ_HISTORY . " WHERE id = %d", $id), ARRAY_N));
			return $refs;
		}
		...
		function update(BibleRefs $refs, $is_read = false)
		{
			...
			if (0 < $user_id)
			{
				...
				{
					$insert = $wpdb->prepare("INSERT INTO $table_name (id, user, verse_start, verse_end, time, is_read) VALUES (%d, %d, %d, %d, NOW(), %d)", $id, $user_id, $unique_ids[0], $unique_ids[1], $is_read);
					$wpdb->query($insert);
				}
			}
		}
		...
		function get_ref_history(BibleRefs $refs, $max = 1, $read = false)
		{
			...
			if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") == $table_name)
			{
				...
				if (0 < $user_id)
				{
					...
					$where_ref = 'AND ' . $refs->sql_where2("$table_name.verse_start", "$table_name.verse_end");
