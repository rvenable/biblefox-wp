...
class BibleSearch {
	...
	public function __construct($text, $ref_str = '', $page = 0, $trans = '', $group = '') {
		...
		else $this->is_group = false;
		if (!empty($ref_str)) $this->set_ref(new BfoxRef($ref_str));
	}
	...
	public function possible_ref() {
		...
		if (!$this->is_group) $ref->add_string($this->ref_str);
		return $ref;
	}
	...
	public function search_text() {
		if ($this->is_group) {
			$ref_str = '';
			$group_str = $this->ref_str;
		}
		else {
			$ref_str = $this->ref_str;
			$group_str = '';
		}
		return bfox_bp_bible_search_url_text($this->text, $ref_str, $group_str);
	}

	public function get_url() {
		return bfox_bp_bible_search_url($this->search_text());
	}
	...
	private function output_group_counts($group, $counts) {
		...
		foreach (BibleMeta::$book_groups[$group] as $child) {
			...
			if (isset(BibleMeta::$book_groups[$child])) list($child_count, $child_content) = $this->output_group_counts($child, $counts);
				...
				$child_content = "<a href='" .
					bfox_bp_bible_search_url($this->text, $ref_str) .
		...
		// Don't use groups for the bible group
		if ('bible' == $group) return array($count,
			"<a href='" .
			bfox_bp_bible_search_url($this->text) .
			"' title='Search for \"$this->text\"'>$ref_str<span class='book_count'>$count</span></a>
			<ul>
				$content
			</ul>");
		...
		return array($count,
			"<a href='" .
			bfox_bp_bible_search_url($this->text, '', $group) .
	...
	public function output_verse_map($book_counts, $group = 'bible') {
	...
	public function output_page_num_description() {
		echo sprintf( __( 'Viewing verse %d to %d (of %d) containing %s', 'bp-bible' ), $this->from_num, $this->to_num, $this->found_rows, $this->description );
	}
	 ...
	 * @param string $group
	 */
	private static function refresh_translation_index(BfoxTrans $trans, $group = 'bible') {
		global $wpdb;
...
function bp_bible_search_list() {
	bfox_bp_locate_template( array( '/bible/search-list.php' ), true );
}
